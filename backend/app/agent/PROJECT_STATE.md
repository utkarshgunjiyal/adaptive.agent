...
| **Latest commit** | `V2 Phase 46.2.3: GitHub Runtime Selection Diagnostics` |
| **Test count** | **877 backend** + **91 frontend** (Vitest) |
...
### Phase 46.2.3 — GitHub Runtime Selection Diagnostics (diagnostic only)
The live UI still executed `list_issues` for "List all my GitHub repositories"
even though 46.2.2 + its tests pass — so the **live orchestration path diverges from
the tested (direct) path**. This phase adds safe, structured observability to
pinpoint where, and **changes no behavior** (no scorer/weights/planner/routing/MCP/
adapter/eligibility/metadata/allowlist change; no GitHub-specific route). New pure
module `app/agent/runtime/diagnostics.py` — every emitter only writes a structured
log (`get_logger("agent.diagnostics")`; the JSON formatter auto-attaches
`request_id`) and mirrors the record onto `run_context.metadata["diagnostics"]`;
all emits are best-effort (a diagnostic failure never affects a run).
- **Events** (all redacted): `agent.runtime_path_selected` (direct / planner /
  deterministic_fast_path + behavior reason + request hash/length + intent labels —
  never raw text), `agent.capability_candidates` (ranked candidates: id, kind,
  provider, server_id, mcp_tool_name, keyword/final score, matched fields/terms,
  enabled, eligible), `agent.capability_selected`, `agent.tool_binding_resolved`
  (decodes `mcp:<server>:<tool>` from handler_ref — proves the id→server/tool map),
  `agent.planner_candidates` + `agent.plan_created` + `agent.plan_tool_resolved`
  (planner path: candidate set, tasks, task→tool resolution), and
  `agent.mcp_tool_invoked` / `agent.mcp_tool_completed` (server/tool, connector
  status, **argument key names only**, duration, item_count, error_code, retry).
- **Redaction by construction:** never the token, Authorization header, argument
  values, raw MCP payload, request/conversation text, or issue/PR bodies.
- **Instrumented (observability only):** `orchestrator` (path + planner prompt +
  plan), `DirectRuntime` (candidates + selected + binding + MCP invoke/complete),
  `PlannerRuntime` (task→tool resolution). MCP adapter/transport/registry untouched
  — the binding is decoded from the ToolSpec `handler_ref`.
- **Helper:** read-only `scripts/diagnose-github-selection.sh` filters the backend
  JSON logs for one run/request and prints only the trace (path, ranking, selection,
  plan, binding, invocation) — refuses to print any token/auth line.
- **Tests.** +7 backend (877): direct-path events, MCP invocation events, planner-path
  events, binding mapping integrity (no cross-binding), eligibility (unavailable →
  no GitHub candidate/invocation), security (no secret/payload/arg-value/raw text),
  and no-behavior-change (selection identical; diagnostics never enter the answer/API).
...
| **Phase 46.3.2.2 ✅** | **Harden GitHub Repository Normalization** | *Done, narrow normalization fix.* When the GitHub MCP payload trimmed the nested `owner` object but sent `full_name="owner/repo"`, `normalize_repository` left `owner=""` (it derived owner only from `raw["owner"]`, never from `full_name`), so `GithubResourceExtractor`'s completeness gate dropped the active repo and only `repo` was published — breaking the next turn's `list_issues` on `missing=["owner"]`. Fix (`github/normalize.py` only): the canonical repository now fills a **missing** `owner`/`name` from a **valid** `full_name` split — exactly one `/`, both segments validated (`validate_owner` + new `validate_repo_name`), both-or-nothing — with **validated structured fields taking precedence** over a conflicting `full_name`, `full_name` recomputed canonically as `owner/name`, and malformed values (`""`, `"repo"`, `"/repo"`, `"owner/"`, `"owner/repo/extra"`, whitespace) never invented. No change to the extractor, publisher, thread store, resolver, argument builder, transport, or routing. +18 backend (962). |
| **Phase 46.3.2.1 ✅** | **Publish Complete GitHub Repository Resources** | *Done.* Fixes a cross-turn publication gap: `"Find my runner-ai repository."` published only `repo` (not `owner`), so the next turn's `"List open issues."` failed with `missing=["owner"]`. Root cause: publication was sourced solely from **pre-execution resolved** resources; when the connector identity is unknown the owner isn't resolvable pre-execution, and the owner present in the successful **normalized output** (`utkarshgunjiyal/runner-ai`) was dropped. Fix: a provider-agnostic post-success **output extractor** seam (`ProviderResourceExtractor` + `ResourceExtractorRegistry`); `GithubResourceExtractor` derives a **complete** active-repository identity (`owner`+`repo`) from the trusted `search_repositories` output — a single result, or the explicitly-requested name confirmed among results — and `ResourcePublisher` merges it over the resolved base (**output authoritative**). A broad multi-repo listing publishes **no** active repo (never guesses); owner comes only from trusted output or resolved identity, never conversation text; failed/missing/ambiguous outcomes still publish nothing; thread/user/provider isolation and explicit-override are preserved. Diagnostics still log types + provenance only. No change to retrieval, selection, schema projection, transport, routing, or retries. +6 backend (944). |
| **Phase 46.3.2 ✅** | **Thread Resource State and Cross-Turn Resolution** | *Done.* Adds execution-state publication + thread-scoped resource memory on top of the 46.3.1 resolver layer, so `"Find my runner-ai repository."` then `"List open issues."` in the same thread resolves `owner`/`repo` automatically — **no LLM**. New `resources/state.py`: `ThreadResourceStore` (in-memory, keyed by `(user_id, thread_id, provider)` → isolation is structural, not best-effort) + `ResourcePublisher` (the post-success seam). `DirectRuntime` gains an additive `execution_observer` seam (default None → byte-identical) invoked **only after a successful execution**; the pipeline surfaces the thread's remembered resources into `ResolutionContext` and attaches the resolved resources to a buildable result for publication. `GithubResourceResolver` cross-turn fill: when the request specifies neither an explicit repo nor account scope (`"my"`), a remembered `owner`/`repo` fills in (source `PRIOR_OUTPUT`); explicit request resources always override memory, and a bare repo name / account listing is never overridden. Resolution priority: request → prior_output → thread_state → connector_identity → cached_context → clarification. Failed / missing / ambiguous outcomes publish nothing (no execution → no publication) and preserve the existing clarification/NEEDS_USER behavior with no MCP call. New safe diagnostic `agent.resources_published` (resource TYPE names + count only — never values). No second provider; GitHub remains the only real one. No change to tool selection, MCP transport, retries, or executor routing. +10 backend (938). |
| **Phase 46.3.1 ✅** | **Provider-Agnostic Resource Resolution Layer** | *Done, behavior-preserving.* Introduces the missing architectural layer between capability selection and argument construction — a **reusable, provider-agnostic Resource Resolver** — so GitHub becomes one implementation and Gmail/Slack/Jira/etc. plug into the same framework. New `app/agent/resources/`: provider-neutral `Resource`/`ResolvedResources`/`ResourceSource`/`ResolutionContext` (models.py); `ResourceResolver` + `ProviderArgumentBuilder` protocols and `ResourceResolverRegistry`/`ArgumentBuilderRegistry` + `provider_of(tool)` = MCP `server_id` (resolver.py); and `ResourceAwareArgumentBuilder` — the pipeline injected as DirectRuntime's `argument_builder` seam (unchanged call site) that runs `resolve → build` and passes through any tool with no registered provider (pipeline.py). GitHub refactored: new `github/resolver.py` (`GithubResourceResolver`, reuses `resources.py`/`identity.py`, emits resources tagged with deterministic `ResourceSource`: explicit→REQUEST, identity→CONNECTOR_IDENTITY, prior repo context→THREAD_STATE); `github/arguments.py` now **consumes `ResolvedResources`** and never parses owners/repos/ids itself (only shapes them onto the discovered schema). New safe diagnostics `agent.resource_resolution_started`/`agent.resource_resolved` (resource TYPES + SOURCE provenance + ambiguity counts only — never values). Resolution priority: request → prior_output → thread_state → connector_identity → cached_context → clarification; LLM never guesses. **Outputs are byte-identical to 46.2.6** (execution-state publication + cross-turn memory land in 46.3.2). No change to scoring, selection, transport, executor routing, retries, allowlist, GitHub ToolSpecs, or the frontend. +14 backend (928). |
| **Phase 46.2.6 ✅** | **GitHub Resource Resolution and Tool Arguments** | *Done.* GitHub execution worked end-to-end but was **semantically wrong**: "List all my GitHub repositories." projected the raw sentence into `{"query": "List all my GitHub repositories."}`, which `search_repositories` ran as a **global** search (30 unrelated repos) instead of an account listing (the verifier uses `{"query": "user:@me"}`). Fix: a deterministic **argument-construction layer** translates NL requests into schema-valid, provider-correct arguments *before* execution. New `github/resources.py` (parse owner/repo/issue#/PR#/"my"), `github/identity.py` (trusted deployment-scoped login via best-effort `get_me` → validated `GITHUB_MCP_OWNER`; public handle, never a secret), `github/arguments.py` (`GithubArgumentBuilder`: account listing → `user:<login>`/`user:@me`; repo tools → `owner`/`repo`(+`state`); reads → `issue_number`/`pull_number`(+`method=get`); search → `author:@me`; snake/camel alias-aware, sets only declared schema keys), and a generic `runtime/arguments.py` result contract. `DirectRuntime` gains an injected `argument_builder` seam (default None → byte-identical): on a missing/ambiguous required resource it **clarifies without any MCP call**; explicit `owner/repo` beats inferred; owners are never guessed from chat text. Deterministic (no LLM). Internal fields (user_id/thread_id/run_id/request_id) never reach a tool (composes with 46.2.4 projection). New safe diagnostics: `agent.tool_arguments_built`/`_validated`/`_rejected` (key names + provenance only). No change to scoring, selection, transport, executor routing, allowlist, or GitHub ToolSpecs. +27 backend (914). |
| **Phase 46.2.5 ✅** | **Restore MCP Execution Routing Under an Internal Executor Override** | *Done.* Live GitHub requests failed with `error_code=unknown_capability` even though `search_repositories` was selected and bound correctly — the error was produced **locally, before any transport call**, at `execution/capability_executor.py:63` (`InternalCapabilityExecutor` has no binding for `mcp.*`). Root cause: the composition root (`main.py`) injects a custom `InternalCapabilityExecutor` as the whole-runtime `capability_executor` override (to wire real document retrieval), and `factory._executor_for` returned any non-`None` override **verbatim**, discarding the by-kind `CompositeCapabilityExecutor`. So MCP tools were mounted for *retrieval* but had no *execution* route and were handed to the internal-only executor. `verify-github-mcp.sh` drives the transport/adapter directly, so it never hit this seam and succeeded. Fix (factory only, ~6 lines): when an override is present **and** multiple kinds are mounted, compose — the override governs `INTERNAL` execution while other kinds (MCP, future) keep their source executors. Internal-only runtimes stay byte-identical (single kind → override returned directly). No change to the MCP client, transport, adapter, planner, retrieval, scoring, or GitHub ToolSpecs. +4 backend (887). |
| **Phase 46.2.4 ✅** | **Preserve MCP Tool Identity & Validate Arguments** | *Done.* The live trace showed `search_repositories` selected/bound/invoked correctly (attempt 1), then a **second** invocation `list_issues` (labelled `retry_attempt=2`) failing `unknown_capability`. Root cause: that second call was **not** a retry — it was `DirectRuntime`'s unconditional **cross-capability fallback** (`matches[1].tool`) firing on the primary's non-retryable failure; and `thread_id`/`user_id` reached the server because `_build_args` injects them and `MCPAdapter` forwarded args verbatim (no schema projection). Fixes: (1) `MCPAdapter` now **projects args strictly** onto the tool's discovered `input_schema.properties` before `call_tool` — internal orchestration fields (`thread_id`/`user_id`/`run_id`/`request_id`) are dropped unless the tool declares them (open schemas still drop internal fields); (2) new additive `ToolSpec.equivalent_capabilities` (empty default) **gates** the cross-capability fallback, so a failed `search_repositories` can never escalate into `list_issues`; (3) transient retries keep re-invoking the same tool (unchanged). No change to scoring, planner prompts, GitHub ToolSpecs, transport, allowlist, normalization, or connector eligibility. +6 backend (883). |
| **Phase 46.2.3 ✅** | **GitHub Runtime Selection Diagnostics** | *Done, diagnostic-only (no behavior change).* Safe structured events (`app/agent/runtime/diagnostics.py`) trace path → candidates → selected → binding → MCP invoke/complete (+ planner candidates/plan/task-resolution). Logged via `agent.diagnostics` (auto `request_id`) and mirrored on `run_context.metadata["diagnostics"]`; fully redacted (no token/headers/arg-values/payloads/raw text). Read-only `scripts/diagnose-github-selection.sh` filters logs for one run. Lets an operator distinguish a ranking vs planner-selection vs task-resolution vs registry-binding vs MCP-invocation error. +7 backend (877). |
...
- **Backend:** **962 passing** (1 benign Starlette deprecation warning),
  `cd backend && python -m pytest`, ~2–3s. Phase 46.3.2.2 adds
  `test_github_normalize` (structured owner unchanged; owner/name/both derived
  from `full_name`; structured-over-`full_name` precedence; malformed `full_name`
  never invents; invalid-structured falls back; normalized repo feeds the
  extractor → owner+repo; multi-repo unchanged; end-to-end cross-turn lookup →
  list-issues; no secret leakage). Phase 46.3.2.1 adds
  `test_thread_resource_state` cases for the output extractor (single-repo lookup
  publishes owner+repo from trusted output under an unknown identity; next-turn
  list-issues resolves both; multi-repo listing publishes no active repo;
  explicit-name-confirmed-among-many; failed lookup publishes nothing;
  output-publication diagnostics carry no values). Phase 46.3.2 adds
  `test_thread_resource_state` (store scope isolation; cross-turn
  find-repo→list-issues; explicit override of memory; no cross-thread/cross-user
  leakage; failed/missing/ambiguous publish nothing; provenance-only publication
  diagnostics). Phase 46.3.1 adds
  `test_resource_resolver` (provider-agnostic layer: resource model accessors,
  `provider_of`/registry dispatch, a fake non-GitHub provider through the same
  pipeline, passthrough for unregistered/internal tools, GitHub resolver source
  provenance, ambiguity, and provenance-only diagnostics) and refactors
  `test_github_arguments` onto the resolver→builder pipeline (outputs unchanged).
  Phase 46.2.6 adds
  `test_github_arguments` (resource resolver, per-tool argument building,
  account-scoped `user:@me`/`user:<login>` listing, explicit/implicit owner,
  issues/PR resolution, missing-context + ambiguity clarification with no MCP
  call, schema validation, identity resolution, security, and the DirectRuntime
  production path). Phase 46.2.5 adds runtime-factory
  regression tests (an internal-execution override composes with — never discards —
  MCP routing; the override still governs internal execution; end-to-end an
  override + MCP manager runs an MCP capability). Phase 46.2.4 adds MCP-adapter argument
  projection tests (internal fields dropped, declared args retained, open-schema
  drops internal fields) and DirectRuntime identity/fallback tests (fallback disabled
  without declared equivalence, transient retry re-invokes the same tool, validation
  failure never switches tools). Phase 46.2.3 adds `test_github_selection_diagnostics`
  (safe event emission across direct/planner paths, binding integrity, eligibility,
  redaction, and no-behavior-change). Phase 46.2.2 adds a DirectRuntime selection
